...
| 📚 Offline dictionaries | Bundled into the HTML (~1,500 entries CN⇄EN + idioms + character meanings): tabs per dictionary, custom entries you can add/remove |
...
## 🈶 Selection → Translation / Explanation

Select any text in the reading area; a floating bar offers:

- **翻译 (Translate)** — opens the dictionary panel and translates the selection (English→Chinese or Chinese→English, auto-detected, word/phrase-level with glosses and a word-by-word gloss line).
- **解释 (Explain)** — Chinese explanation: idiom lookup, word segmentation with glosses, per-character breakdown, and a list of characters not (yet) in the dictionary.
...
# run the logic unit tests (749 checks: segmentation, encodings,
